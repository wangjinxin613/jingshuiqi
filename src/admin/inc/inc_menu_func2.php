...
$headTemplet = "<li class='layui-nav-item'><a href='javascript:;'><i class='iconfont'>&#xe61f;</i><cite>~channelname~</cite><span class='layui-nav-more'></span></a><dl class='layui-nav-child'>";

$footTemplet = "</dl></li>";

$itemTemplet = "<dd>~link~</dd>";
    ...
    foreach($dtp->CTags as $i=>$ctag)
    {
        if($ctag->GetName()=='top' && ($ctag->GetAtt('rank')=='' || TestPurview($ctag->GetAtt('rank')) ))
        {
            ...
            foreach($dtp2->CTags as $j=>$ctag2)
            {
                ...
                {
                    $link = "<a href='".$ctag2->GetAtt('link')."' target='".$ctag2->GetAtt('target')."'><i class='iconfont'>&#xe630;</i><cite>".$ctag2->GetAtt('name')."</cite></a>";
                    if($ischannel=='1')
                    {
