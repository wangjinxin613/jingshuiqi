...
else if($dopost=="delArchives")
{
    ...
    if($fmdo=='yes')
    {
        ...
        if($qstr=='')
        {
            layerMsg("参数无效！",$ENV_GOBACK_URL,0);
            exit();
        }
        $qstrs = explode("`",$qstr);
        $okaids = Array();
        ...
        layerMsg("成功删除指定的文档！",$ENV_GOBACK_URL,1);
        exit();
    }
    else
    {
...
else if($dopost=='return')
{
    ...
    layerMsg("成功还原指定的文档！","recycling.php",1);
    exit();
}
/*-----------------------------
//清空文档
...
else if($dopost=='clear')
{
    ...
    if($fmdo=='yes')
    {
        ...
        layerMsg("成功删除指定的文档！","recycling.php",1);
        exit();
    }
    else
    {
        ...
        if(empty($num))
        {
            ShowMsg("对不起，未发现相关文档！","recycling.php",0);
            exit();
        }
